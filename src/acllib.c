...
#define ACL_ASSERT_HWND                                                                                                \
    ACL_ASSERT(g_hWnd != 0, "You should call function \"initWindow(...)\" befor use function \"" __FUNCTION__ "\"")
#define ACL_ASSERT_BEGIN_PAINT                                                                                         \
    ACL_ASSERT(g_hmemdc != 0, "You should call function \"beginPaint()\" befor use function \"" __FUNCTION__ "\"")
        ...
        case WM_CREATE:
            {
                HDC hdc;
                hdc = GetDC(hwnd);
                g_hbitmap = CreateCompatibleBitmap(hdc, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN));
        ...
        case WM_PAINT:
            {
                ...
                BitBlt(hdc, 0, 0, rect.right - rect.left, rect.bottom - rect.top, g_hmemdc, 0, 0, SRCCOPY);
                DeleteDC(g_hmemdc);
                g_hmemdc = 0;
                EndPaint(hwnd, &ps);
                break;
...
void initWindow(const char* wndName, int x, int y, int width, int height)
{
    ...
    g_hWnd = CreateWindowA(g_wndClassName,
                           wndName,
...
void clearDevice(void)
{
    ...
    BitBlt(g_hmemdc, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), g_hmemdc, 0, 0, WHITENESS);
}
...
void updateFont()
{
    if (g_font)
        DeleteObject(g_font);
    g_font = CreateFontA(g_textSize, 0, 0, 0, 700, 0, 0, 0, 0, 0, 0, 0, 0, g_fontName);
